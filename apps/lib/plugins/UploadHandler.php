...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
    	...
    	$this->error_messages = array(
	        ...
	        'max_width' => t('Hình ảnh vượt quá chiều rộng tối đa'), //Image exceeds maximum width
	        'min_width' => 'Image requires a minimum width',
	        'max_height' => t('Hình ảnh vượt quá chiều cao tối đa'), //Image exceeds maximum height
	        'min_height' => t('Hình ảnh yêu cầu 1 chiều cao tối thiểu')
	    );
    ...
    // this is where we collect all the response data and bind to template
    ...
    public function get($print_response = true) {
        ...
        if ($file_name) {
        	...
        	$files = array($file);
            $response = array(            	
                substr($this->options['param_name'], 0, -1) => $file
            );
        } else {
        ...
        // callback function
        if($this->options['get_callback']){ 
        	$this->options['get_callback']($files, $this->options['custom_data'], $response);
        }
    ...
    public function post($print_response = true) {
        ...
        // callback function
        if($this->options['upload_callback']){      
			$file_data = array();
			foreach($files as $file)
				if(!isset($file->error)) // process successfull file only
					array_push($file_data, $file);
        	$this->options['upload_callback']($file_data, $this->options['custom_data'], $response);
        }
    ...
    public function delete($print_response = true) {
        ...
        if($this->options['delete_callback']){
        	$file = new stdClass();
        	$file->name = $file_name;
        	$files = array($file); // support multi delete later
        	$this->options['delete_callback']($files, $this->options['custom_data'], $response);
        }
