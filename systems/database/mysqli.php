...
$db_link = null;
$db_link_collection = array();

/**
 * We keep multi-connection until script is done
 * each account can be added to each database
 * when change server, always change account information
 * So if you want to close previous connection, just call db_close then reconnect
 * $db_name must be case-sensitive
 * @return void
 */
function db_connect($db_name = DB_NAME, $db_username = DB_USERNAME, $db_password = DB_PASSWORD, $db_server = DB_SERVER) {
    global $db_link_collection;
    global $db_link;
    // key is SERVER.DATABASE
    if (!isset($db_link_collection[$key = $db_server . '.' . $db_name])) {
        $db_link_collection[$key] = mysqli_connect($db_server, $db_username, $db_password, $db_name);
        mysqli_set_charset($db_link_collection[$key], CHARSET);
    }
    $db_link = $db_link_collection[$key];
}
 ...
 * @global null $db_link
 */
function db_close($db_name = DB_NAME, $db_server = DB_SERVER) {
    global $db_link_collection;
    global $db_link;
    $db_link = $db_link_collection[$key = $db_server . '.' . $db_name];
    // no need to unset, this variable will show out that connection is closed
    mysqli_close($db_link);
    // remove from collection
    unset($db_link_collection[$key]);
}
...
/**
 * For little params, and their priorities are difference, put default value is faster than specific key
...
function db_query($sql, $one_field = false) {
    global $db_link;
    // return null means sql has problem, array emtpy means empty result
    $ret = null;
...
function db_query_one($sql, $one_field = false) {
    ...
    $ret = null;
    if ($result = mysqli_query($db_link, $sql)){
        $ret = $one_field ? mysqli_result($result, 0, 0) : mysqli_fetch_assoc($result);
        // clear memory as soon as possible
        mysqli_free_result($result);
    }
    return $ret;
...
/**
 * Implementing for approximated mysql_result 
 * @param type $res
 * @param type $row
 * @param type $field
 * @return type
...
/**
 * Don't use prepare statement unless you have to run thousands of queries
 * @param $sql
 * @return resource
 */
