(function($) {
    jQuery.fn.html5_upload = function(options) {
        ...
        function upload(event) {    
            var $this = $(this)
            // may be multiple file
            var files = this.files;
            // may be files come from drop zone
            if(files.length === 0 && $this.data('files')){
            ...
            function upload_file(number) {
                // even we have no files uploaded, we still have to trigger onFinish to tell the form to trigger data submit that delay by files
                if (number == total) {
                ...
                if (!options.sendBoundary) {
                    ...
                    if (window.FormData) {//Many thanks to scottt.tw
                        ...
                        // function to get data in realtime
