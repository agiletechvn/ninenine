        ...
        SelectParser.prototype.add_option = function(option, group_position, group_disabled) {
            if (option.nodeName.toUpperCase() === "OPTION") {
                if (option.text !== "") {
                    ...
                    var html = dummyTextarea.html(option.innerHTML).val();
                    this.parsed.push({
                        array_index: this.parsed.length,
                        options_index: this.options_index,
                        value: option.value,
                        text: dummyDiv.html(html).text(),
                        html: html, // hack for decode
...
(function() {
    ...
    AbstractChosen = (function() {
        ...
        AbstractChosen.prototype.update_container_position = function() {
            var offset = App.getOffset(this.form_field_jq, this.parentEl);
            this.container.css(offset);
        };

        AbstractChosen.prototype.results_update_field = function() {

            // re-calculate width, after first time calculate, select now is hidden  
            this.f_width = this.form_field_jq.is(':hidden')
                    ? (this.form_field_jq.parent().width() - get_side_border_margin(this.form_field_jq.parent()))
                    : this.form_field_jq.outerWidth();

            // update width for container only, then children will flow out
            this.container.width(this.options.width || this.f_width);
...
(function() {
    ...
    Chosen = (function(_super) {
        ...
        Chosen.sequence = 1000;
        Chosen.sequenceMax = 1010;
        Chosen.sequenceMin = 101;

        __extends(Chosen, _super);

        function Chosen() {
            // run from very first  
            // by default, the next chosen will have a smaller zindex, so it will be hidden for prev to show
            Chosen.sequence--;
            if (Chosen.sequence < Chosen.sequenceMin) {
                Chosen.sequence = Chosen.sequenceMax;
            }
        ...
        Chosen.prototype.update_multiple_change = function() {
            var chosen = this;
            setTimeout(function() {
                var selectHeight;
                if (chosen.search_choices &&
                        chosen.form_field_jq.height() !== (selectHeight = chosen.search_choices.height())) {
                    // update in chain
                    chosen.form_field_jq.height(selectHeight);

                    // update other chosen from uiList
                    App._uiList.forEach(function(item) {
                        if (item && item[0] !== chosen.form_field_jq[0] && item.attr('ui-type') === 'chosen-select') {
                            item.data('chosen').update_container_position();
                        }
                    });
                }
            });
        ...
        Chosen.prototype.set_up_html = function() {
            ...
            this.form_field_jq.css('visibility', 'hidden');
            ...
            this.container.css('z-index', Chosen.sequence);
            ...
            if (!this.options.nativeScroll) {
                ...
                //console.log(this.search_results.css('max-height'), this.container.width());
            }

            var chosen = this;
            // require-visible attribute will not help, because this ui must follow state of input
            this.options.observeDelay = this.options.observeDelay || 100;
            if (this.parentEl[0] !== document.body) {
                var elapsed = 0;
                this.form_field_observe_timer = setInterval(function() {
                    // so fast, don't care, hide it as soon as possible so you cant see :D
                    if (chosen.form_field_jq.is(':hidden')) {
                        chosen.container.css({
                            "left": "-9000px"
                        });
                        this.form_field_jq_hidden = true;

                    } else {

                        elapsed++;
                        if (elapsed > chosen.options.observeDelay) {
                            elapsed = 0;
                            if ((this.form_field_jq_hidden === undefined) || this.form_field_jq_hidden) {
                                // only update when change, this is slower than above code
                                // update this first
                                chosen.update_container_position();
                            ...
                            this.form_field_jq_hidden = false;
                        }

                    }

                }, 1);
                // this function not require change immediately, just adjust position, so interval no need to be small
        ...
        Chosen.prototype.register_observers = function() {
            ...
            this.form_field_jq.bind("liszt:updated", $.proxy(_this.results_update_field, _this))
                    ...
                    .on("chosen:close.chosen", $.proxy(_this.input_blur, _this));
        ...
        Chosen.prototype.container_mousedown = function(evt) {
            ...
            /*var target_closelink;
             ...
             this.results_show();
             } else if (!this.is_multiple && evt && ((evt.target === this.selected_item[0]) || $(evt.target).closest("a.chzn-single").length)) {
             evt.preventDefault();
             this.results_toggle();
             }
             return this.activate_field();
            ...
            // when input search is not active, empty choice will show default search text

            //   console.log(this.is_disabled,this.results_showing,this.active_field)
            ...
            if (!this.is_disabled) {
                ...
                if (!((evt != null) && ($(evt.target)).hasClass("search-choice-close"))) {
                    ...
                    if (!this.active_field) {
                        ...
                        this.results_show();
                    } else if (!this.is_multiple && evt && (($(evt.target)[0] === this.selected_item[0]) || $(evt.target).closest("a.chosen-single").length)) {
                        evt.preventDefault();
                        this.results_toggle();
                    }
                    return this.activate_field();
        ...
        Chosen.prototype.test_active_click = function(evt) {
            ...
            //var testTarget;
            /*if (!this.is_multiple && (testTarget = $(evt.target).closest('.chzn-single')).length) {
             this.active_field = false;
             //console.log(this.active_field);
             return true;
             } else {*/
        ...
        Chosen.prototype.choice_destroy = function(link) {
            if (this.result_deselect(link.attr("rel"))) {
                ...
                if (this.is_multiple) {
                    //var selectHeight = this.search_choices.height();  
                    //this.form_field_jq.height(selectHeight);
                    this.update_multiple_change();
                }
            }
        };
        ...
        Chosen.prototype.result_select = function(evt) {
            ...
            if (this.result_highlight) {
                ...
                if (this.is_multiple || this.form_field_jq.val() !== this.current_value) {
                    // this help the select has the correct height, to flow other elements
                    if (this.is_multiple) {
                        //var selectHeight = this.search_choices.height();  
                        //this.form_field_jq.height(selectHeight);
                        this.update_multiple_change();
        ...
        Chosen.prototype.result_deselect = function(pos) {
            ...
            if (!this.form_field.options[result_data.options_index].disabled) {
                ...
                // result_deselect is run from choice_destroy only
                // but result_select run from many 
                /*if(this.is_multiple){
                 //var selectHeight = this.search_choices.height();
                 //this.form_field_jq.height(selectHeight);
                 this.update_multiple_change();
        ...
        Chosen.prototype.destroy = function() {
            // clear event and empty markup
            $(document).off("click.chosen", this.click_test_action);
            if (this.parentEl[0] !== document.body) {
                clearInterval(this.form_field_observe_timer);
            }
            // when remove, it will empty, and all event bound to it will be removed too
            this.container.remove();
            this.form_field_jq.removeData('chosen');
            this.form_field_jq.css('visibility', '');
        }
    ...
    get_side_border_margin = function(elmt) {
        return parseInt(elmt.css('margin-left')) + parseInt(elmt.css('margin-right'));
    };
